...
Live2DManager.prototype._configureLoadedModel = async function(model, modelPath, options, loadToken) {
    ...
    // 启用鼠标跟踪（始终启用监听器，但内部根据设置决定是否执行眼睛跟踪）
    // 注意：enableMouseTracking 包含悬浮菜单显示/隐藏逻辑，必须始终启用
    const shouldEnableMouseTracking = options.mouseTracking !== false;
    console.log(`[Live2D] 鼠标跟踪检查: window.mouseTrackingEnabled=${window.mouseTrackingEnabled}, shouldEnable=${shouldEnableMouseTracking}`);
    if (shouldEnableMouseTracking) {
